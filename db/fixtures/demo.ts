...
export async function insertDemoFixture(sql: Sql): Promise<void> {
  ...
  const dashboardSchema=(await sql<{ready:boolean}[]>`SELECT to_regclass('public.patients') IS NOT NULL AND EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='appointments' AND column_name='attendance') AS ready`)[0]?.ready;
  if(!dashboardSchema){const publicToken=tokenHash(demoIds.clinic);await sql`INSERT INTO public_booking_tokens (organization_id, site_id, token_hash, active, rate_limit_per_minute) VALUES (${demoIds.clinic}, ${demoIds.providencia}, ${publicToken}, true, 20) ON CONFLICT (token_hash) DO UPDATE SET active = true, revoked_at = null, updated_at = now()`;return;}
  const day=todayInSantiago();
  const instant=(offset:number,time:string)=>{const d=addDaysLocal(day,offset);return santiagoLocalToUtc(new Date(Date.UTC(d.year,d.month-1,d.day,12)),time)};
  const patientRows=[
    ["10000000-0000-4000-8000-000000000301","Ana","Paz"],["10000000-0000-4000-8000-000000000302","Bruno","Solís"],["10000000-0000-4000-8000-000000000303","Camila","Reyes"],["10000000-0000-4000-8000-000000000304","Diego","Muñoz"]
  ] as const;
  for(const [id,first,last] of patientRows)await sql`INSERT INTO patients(id,organization_id,first_name,last_name,rut,consent_granted,created_at) VALUES(${id},${demoIds.clinic},${first},${last},${`99.999.${id.slice(-3)}-9`},true,${id.endsWith("303")||id.endsWith("304")?instant(0,"08:30"):instant(-20,"10:00")}) ON CONFLICT(id) DO UPDATE SET first_name=EXCLUDED.first_name,last_name=EXCLUDED.last_name,created_at=EXCLUDED.created_at,updated_at=now()`;
  const dashboardAppointments=[
    ["10000000-0000-4000-8000-000000000401",0,"09:00","09:45",demoIds.proOne,"10000000-0000-4000-8000-000000000301","Ana Paz","confirmed",null],
    ["10000000-0000-4000-8000-000000000402",0,"10:00","10:30",demoIds.proTwo,"10000000-0000-4000-8000-000000000302","Bruno Solís","pending",null],
    ["10000000-0000-4000-8000-000000000403",0,"11:00","11:45",demoIds.proOne,null,"Carolina Fuentes","confirmed",null],
    ["10000000-0000-4000-8000-000000000404",0,"12:00","12:30",demoIds.proTwo,null,"Álvaro Soto","cancelled",null],
    ["10000000-0000-4000-8000-000000000405",0,"15:00","16:00",demoIds.proOne,"10000000-0000-4000-8000-000000000303","Camila Reyes","pending",null],
    ["10000000-0000-4000-8000-000000000406",0,"16:30","17:00",demoIds.proTwo,null,"Valeria Núñez","confirmed",null],
    ["10000000-0000-4000-8000-000000000407",-1,"09:00","09:45",demoIds.proOne,"10000000-0000-4000-8000-000000000301","Ana Paz","confirmed","attended"],
    ["10000000-0000-4000-8000-000000000408",-1,"10:00","10:30",demoIds.proTwo,"10000000-0000-4000-8000-000000000302","Bruno Solís","confirmed","attended"],
    ["10000000-0000-4000-8000-000000000409",-2,"11:00","11:30",demoIds.proOne,"10000000-0000-4000-8000-000000000303","Camila Reyes","confirmed","missed"]
  ] as const;
  for(const [id,offset,start,end,pro,patientId,name,status,attendance] of dashboardAppointments)await sql`INSERT INTO appointments(id,organization_id,site_id,professional_membership_id,box_id,patient_id,patient_name,patient_contact,starts_at,ends_at,status,attendance,cancellation_reason,source) VALUES(${id},${demoIds.clinic},${demoIds.providencia},${pro},${demoIds.boxOne},${patientId},${name},'FICTICIO',${instant(offset,start)},${instant(offset,end)},${status},${attendance},${status==='cancelled'?'Cancelación demo':null},'seed-dashboard') ON CONFLICT(id) DO UPDATE SET patient_id=EXCLUDED.patient_id,starts_at=EXCLUDED.starts_at,ends_at=EXCLUDED.ends_at,status=EXCLUDED.status,attendance=EXCLUDED.attendance,updated_at=now()`;
  ...
  const movements=[
   ["10000000-0000-4000-8000-000000000601",0,"10:15","10000000-0000-4000-8000-000000000301",demoIds.proOne,"payment",45000],["10000000-0000-4000-8000-000000000602",0,"12:15","10000000-0000-4000-8000-000000000302",demoIds.proTwo,"payment",60000],["10000000-0000-4000-8000-000000000603",0,"15:30","10000000-0000-4000-8000-000000000303",demoIds.proOne,"payment",30000],["10000000-0000-4000-8000-000000000604",0,"16:00","10000000-0000-4000-8000-000000000301",demoIds.proOne,"charge",120000],["10000000-0000-4000-8000-000000000605",-1,"11:00","10000000-0000-4000-8000-000000000301",demoIds.proOne,"payment",40000],["10000000-0000-4000-8000-000000000606",-1,"14:00","10000000-0000-4000-8000-000000000302",demoIds.proTwo,"payment",50000]
  ] as const;
  for(const [id,offset,time,patient,pro,kind,amount] of movements)await sql`INSERT INTO billing_movements(id,organization_id,patient_id,site_id,professional_membership_id,actor_membership_id,kind,amount_clp,status,reason,evidence,created_at) VALUES(${id},${demoIds.clinic},${patient},${demoIds.providencia},${pro},${demoIds.admin},${kind},${amount},'posted','Movimiento demo dashboard',${JSON.stringify({reference:`DASH-${id.slice(-3)}`,fictional:true})}::jsonb,${instant(offset,time)}) ON CONFLICT(id) DO NOTHING`;
  const estimates=[['10000000-0000-4000-8000-000000000701','10000000-0000-4000-8000-000000000711','10000000-0000-4000-8000-000000000301','sent',185000],['10000000-0000-4000-8000-000000000702','10000000-0000-4000-8000-000000000712','10000000-0000-4000-8000-000000000302','approved',240000]] as const;
  for(const [estimateId,versionId,patient,state,total] of estimates){await sql`INSERT INTO estimates(id,organization_id,patient_id,author_membership_id,current_version,state) VALUES(${estimateId},${demoIds.clinic},${patient},${demoIds.proOne},1,${state}) ON CONFLICT(id) DO UPDATE SET state=EXCLUDED.state,updated_at=now()`;await sql`INSERT INTO estimate_versions(id,organization_id,estimate_id,version,state,total_clp,snapshot) VALUES(${versionId},${demoIds.clinic},${estimateId},1,${state},${total},${JSON.stringify({fictional:true})}::jsonb) ON CONFLICT(id) DO NOTHING`;}
